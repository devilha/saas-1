...
//         }
    ...
    public IniSection[] getSectionsByName(String name){    	
    	ArrayList<IniSection> iniSections = new ArrayList<IniSection>();
    	...
    	if( iniSections.size() > 0 ){
    		IniSection[] iss = new IniSection[iniSections.size()];
    		for( int i = 0; i < iniSections.size(); i ++ )
    			iss[i] = iniSections.get( i );
    		return iss;
    	}
    	else return null;
    }
     ...
     *         both IniFiles.
     * @see com.bfuture.util.ini.IniSection#merge(com.bfuture.util.ini.IniSection)
     */
