	...
	<script>
		...
		$(function(){
			...
			$('#supcodeSelect').combogrid({
				...
				queryParams : {
					data :obj2str(
						{		
							...
							ACTION_MANAGER : 'infSupinfoManager',		 
							list:[{
				...
				fit:true,							
				columns:[[
					{field:'SUPID',title:'供应商编码',width:80,sortable:true,align:'center'},
					{field:'SUPNAME',title:'供应商名称',width:240,sortable:true}		
				]],				
				pagination:true,
				onSelect : function( row ){				
					if( !row ) return;
					var supcode = $('#supcodeSelect').combogrid('getValue');					
					$('#supcode').val( supcode );
					...
					$.post( 'JsonServlet',				
						...
						function(data){ 
		                    if(data.returnCode == '1' ){ 
		                    	if( data.rows ){
		                    		var supinfo = data.rows[0];
		                    		$('#linkman').attr('value', supinfo.supcont );
		                    		$('#linkmantel').attr('value', supinfo.supcontphone );
		                    		$('#email').attr('value', supinfo.supcontemail );
		                    		$('#fax').attr('value', supinfo.supfax );
		                    		$('#bank').attr('value', supinfo.supbank );
		                    		$('#account').attr('value', supinfo.supaccount );
		                    		$('#zipcode').attr('value', supinfo.supzip );
		                    		$('#legalperson').attr('value', supinfo.suplegal );
		                    		$('#address').attr('value', supinfo.supadd );
		                    	} 
		                    }else{ 
		                        $.messager.alert('提示','获取供应商信息失败!<br>原因：' + data.returnInfo,'error');
		                    } 
		            	},
		            	'json'
		            );
			...
			$('#sutype').change(function(){ 
				var newVal = $(this).children('option:selected').val(); 
				
				if( newVal == 'S' ){
					$('#supcodeSelect').combogrid('enable');
				}
				else{
					$('#supcodeSelect').combogrid('disable');
				}
		...
		function add(){
			...
			$('#sutype').val('S');
			$('#supcodeSelect').combogrid('setValue', '');
			...
			$.post( 'JsonServlet',				
				{
					data : obj2str({		
							...
							list:[ getFormData( 'userManager_form' ) ]
					})
		...
		<div class="easyui-layout" fit="true"> 
			<div region="center" border="false" style="padding:10px;background:#fff;border:1px solid #ccc;"> 
				<table id="userManager_form" class="tableClass">
					<tr>
						<td>用户编码：</td>
						<td><input required="true" id="sucode" name="sucode" type="text" disabled="disabled" /></td>
						<td>用户名称：</td>
					...
					<tr>
						...
						<td>
							...
							<input id="supcodeSelect" value=""/>
							<input id="supcode" name="supcode" type="hidden" value=""/>							
						</td>
					</tr>
					<tr>
						<td>密码提示问题：</td>
