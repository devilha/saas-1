...
<form id="uploadForm" action="file.jsp?spbm=<%=spbm%>">
  <table border="0" cellspacing="1" align="center" class="fu_list">
    <thead>
      <tr>
    ...
    <tbody>
      <tr>
        ...
        <td id="idFileTd" style="display:block;"><a href="javascript:void(0);" class="files" id="idFile"></a></td>
      </tr>
      <tr>
      ...
      <tr>
        <td colspan="2" style="color:gray">温馨提示：最多可同时上传5个文件，单个文件不得超过5M，只允许上传gif,jpg,png,GIF,JPG,PNG文件。 </td>
      </tr>
      <tr>
...
var hasFileuUpload = parseInt(parent.getFileNum());
...
var fu = new FileUpload("uploadForm", "idFile", { 
	...
	onIni: function(){
		...
		if(this.Files.length){
			...
			var fileUploadNum = parent.getFileNum();
			if(fileUploadNum>0){
				 arrRows.push(["<font color='gray'>你已经上传"+fileUploadNum+"个文件，还可以上传"+(5-fileUploadNum)+"个文件</font>", "&nbsp;"]);
			}else if(fileUploadNum == 5){
				 document.getElementById("idFileTd").style.display="none";
			}else{
...
function Finish(msg){ 
    ...
    if(msg=="-1"){
    	...
    	alert("同名附件已经存在，请重新命名上传附件");
    }else if(msg=="-3"){
    	alert("上传失败");
    }else{
    	alert("上传成功");
    	parent.savaFileNum(msg);
    }
}
</script>
</body>
