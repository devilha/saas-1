	...
	$(function(){
			...
			$('#popInfoList').datagrid({
				...
				columns:[[
						...
						{field:'PPGDBARCODE',title:'商&nbsp;品&nbsp;条&nbsp;码',width:140,align:'center'},	
						{field:'PPGDNAME',title:'商&nbsp;品&nbsp;名&nbsp;称',width:150,align:'center'},	
						{field:'PPGDID',title:'商&nbsp;品&nbsp;编&nbsp;码',width:100,align:'center'},	
		...
		function reloadgrid(){
			...
			searchData['ppsupid'] = supcode;  		// 供应商编码
   	...
   	function showPopInfoDet(num){
		$.post( 'JsonServlet',				
					...
					function(data){ 
	                    if(data.returnCode == '1' ){
             				 $("#supname_").html(data.rows[0].PPSUPNAME == null || data.rows[0].PPSUPNAME == undefined ? "供&nbsp;应&nbsp;&nbsp;商：" : "供&nbsp;应&nbsp;&nbsp;商："+ data.rows[0].PPSUPNAME);
	                    	 ...
	                    	 $("#pphfyj_").html(data.rows[0].PPHFYJ == null || data.rows[0].PPHFYJ == undefined ? "回&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;复：" : data.rows[0].PPHFYJ == "0"? "回&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;复：未审核":data.rows[0].PPHFYJ == "1"?"回&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;复：通过":data.rows[0].PPHFYJ == "2"?"回&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;复：未通过":"");
	                    	 $("#ppcgyj_").html(data.rows[0].PPCGYJ == null || data.rows[0].PPCGYJ == undefined ? "采购意见：" : "采购意见："+data.rows[0].PPCGYJ );
	...
	*/
	function exportExcel(){
			...
			if(User.sutype == 'L'){
				...
				searchData['cnTitle'] = ['商品条码','商品','商品编码','门店','发送时间','回复意见','采购意见'];
			} 
			searchData['ppsupid'] = supcode;  		// 供应商编码
			searchData['sgcode'] = User.sgcode;  // 实例编码
...
<table id="popInfoSearch" width="800px" border="0" cellspacing="0" cellpadding="0" style="margin-left:0; font-size:12px;line-height:30px;">
  <tr>
    <td colspan="6"><span style="color:#33CCFF; margin:0;">促销申请表查询</span></td> 
  </tr>
  <tr>
    <td align="left" class="label_style">商品名称：</td>
...
<table width="800px" style="margin:40px auto;line-height:20px; text-align:left; border:0;font-size:14px;">
  <tr>
    <td align="left" style="color:#33CCFF;">价格管理-&gt;促销变价申请</td>
    <td align="left">&nbsp;</td>
  </tr>
    <tr>
    <td align="left">&nbsp;</td>
